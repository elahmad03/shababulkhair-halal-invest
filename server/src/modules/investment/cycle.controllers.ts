...
export const createVenture = catchAsync(async (req: AuthenticatedRequest, res: Response) => {
  ...
  const venture = await CycleService.createVenture(cycleId, input);
  res.status(201).json(successResponse(venture, "Business venture created"));
});

export const recordVentureProfit = catchAsync(async (req: AuthenticatedRequest, res: Response) => {
  const { ventureId } = req.params;
  const { profitRealizedNaira } = recordVentureProfitSchema.parse(req.body);
  const result = await CycleService.recordVentureProfit(ventureId, profitRealizedNaira);
  res.status(200).json(successResponse(result, "Venture profit recorded"));
});

export const recordLedgerEntry = catchAsync(async (req: AuthenticatedRequest, res: Response) => {
  const recordedById = req.user!.userId;
  const input = recordLedgerEntrySchema.parse(req.body);
  const entry = await CycleService.recordLedgerEntry(recordedById, input);
