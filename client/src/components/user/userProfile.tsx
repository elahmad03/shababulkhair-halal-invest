...
const UserProfile: React.FC = () => {
  ...
  const { profile, loading, error, updateLoading } = useAppSelector(state => state.user);
  const { data: kycData, loading: kycLoading } = useAppSelector(state => state.kyc);
  ...
  if (error) {
    return (
      <Alert variant="destructive" className="max-w-4xl mx-auto">
        ...
        <AlertDescription>
          {error}
          <Button variant="outline" className="ml-4" onClick={() => dispatch(clearUserError())}>
            Clear Error
          </Button>
        </AlertDescription>
      </Alert>
          ...
          <div className="space-y-2 flex-1">
            ...
            <div className="flex flex-wrap gap-2">
              ...
              <span className={`inline-flex items-center rounded-full px-3 py-1 text-sm font-medium ${
                profile.identity?.verified ? 'bg-green-100 text-green-800' : 'bg-yellow-100 text-yellow-800'
              }`}>
