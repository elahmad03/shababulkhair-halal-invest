...
function processWalletData(userId: number): WalletScreenData {
  ...
  const userWalletRaw = wallets.find(w => w.userId === userId) as unknown as ParsedWallet;
  const balance = userWalletRaw ? userWalletRaw.balance : 0.00;
  ...
  return (
    <section className="p-4 md:p-8 max-w-7xl mx-auto space-y-6 md:space-y-8">
      ...
      <div className="bg-zinc-100 dark:bg-zinc-800 rounded-lg shadow p-4" >
         ...
         <div className='mb-4'> 
          <TotalBalanceBox 
          totalCurrentBalance={walletData.balance}
        />
        </div>
